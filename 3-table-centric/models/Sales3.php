 ...
 * @author jim
 */
class Sales3 extends Sales {

    // populate our internal data
    function _load() {
        $this->_root = simplexml_load_file('data/04/sales04-table.xml');
    }
    ...
    function findLargest($country, $vtype) {
        ...
        $quarter = 0;
        $units = 0;
        foreach ($this->_root->sold as $sold) {
            if (($sold['country'] == $country) && ($sold['type'] == $vtype))
                if ($sold > $units) {
                    $units = (int) $sold;
