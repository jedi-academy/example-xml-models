...
class Lab07 extends CI_Controller {
    ...
    /**
     * Process the source-centric XML, with embedded codes, building 3D array for presentation
     */
    function six() {
        // build 3-D array
        $xml = simplexml_load_file('./data/04/sales04-embedded.xml');
        ...
        // let's also extract & pass on the countries and categories
        $countries = array();
        foreach ($xml->countries->country as $country)
            $countries[(string) $country['code']] = (string) $country;
        ...
        $data['vehicle_types'] = $categories;
        $data['pagetitle'] = 'COMP4711 Lab 7 Solution - source-centric';
        $data['results'] = $results;
        $data['subtitle'] = 'Process Source-centric XML';
        $data['pagebody'] = 'lab07report6';
        $data['selected'] = '/lab07/one';
        $data['tabs'] = $this->tabs;
        $data['data'] = &$data;
